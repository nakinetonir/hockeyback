...
export async function getDashboardSummary() {
  const [totalMatches, totalPlayers, totalGoalies, topScorers, topAssisters, topGoalies, topShotTeams] = await Promise.all([
    ...
    TeamShotTotalModel.find().sort({ shots_for: -1, shots_against: 1, team: 1 }).limit(10).lean()
  ]);
